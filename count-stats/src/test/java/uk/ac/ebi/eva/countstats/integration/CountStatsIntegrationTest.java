...
public class CountStatsIntegrationTest {
    ...
    public void testSaveCount() throws Exception {
        ...
        String responseString1 = mvc.perform(post("/v1/count")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(count1)))
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
        String responseString22 = mvc.perform(post("/v1/count")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(count2)))
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();

        long id1 = objectMapper.readTree(responseString1).get("id").longValue();
        ...
        long id2 = objectMapper.readTree(responseString22).get("id").longValue();
